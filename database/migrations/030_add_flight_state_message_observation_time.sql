...
COMMENT ON COLUMN flight_states.message_observed_at IS
    'Optional latest provider message/contact time for this source, aircraft, and position identity; may be newer than the position observation, must not replace position freshness, and replay conflicts may monotonically refresh it without creating a new position state.';
COMMIT;
