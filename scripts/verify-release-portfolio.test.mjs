...
test('README records current engineering closure and remaining v1 boundary', async () => {
  ...
  assert.match(readme, /FRONTEND_VISUAL_POLISH_V2=CLOSED/)
...
test('frontend visual polish closure preserves zero-budget truth and exact-head evidence', async () => {
  const closure = await text('docs/196_FRONTEND_VISUAL_POLISH_V2_CLOSURE.md')

  assert.match(closure, /Status: CLOSED/)
  assert.match(closure, /PR_HEAD=f1d2ba16cec215b217955fbcd049dd8b728562c4/)
  assert.match(closure, /MERGE_SHA=ce7ee7ab0b95655fff7a1b546e277a4e1c0b842f/)
  assert.match(closure, /PLAYWRIGHT_BROWSER_RESULT=20_PASSED/)
  assert.match(closure, /PLAYWRIGHT_FLAKY_RETRIES=0/)
  assert.match(closure, /ZERO_BUDGET_DATA_BOUNDARY=ENFORCED/)
  assert.match(closure, /UNSUPPORTED_FR24_FIELDS=NOT_RENDERED/)
  assert.match(closure, /PIXEL_GOLDEN_VISUAL_REGRESSION=NOT_ADOPTED_NONBLOCKING/)
  assert.match(closure, /PRODUCTION_PROVIDER_RECOVERY=OPEN_RUNTIME_VALIDATION/)
