...
assert(config.triggers.crons.includes('17 */2 * * *'), 'Worker must define the free-tier two-hour primary schedule')
assert(config.triggers.crons.includes('19 */2 * * *'), 'Worker must define the free-tier two-hour watchdog')
assert(!config.triggers.crons.includes('17,47 * * * *'), 'Worker must not retain the validated-too-aggressive 30-minute primary')
...
assert(source.includes("primaryCron: '17 */2 * * *'"), 'Worker source default must match free-tier primary cadence')
