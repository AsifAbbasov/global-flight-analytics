...
test('production Worker disables preview URLs while keeping workers.dev', () => {
  ...
  assert.ok(config.name.length <= 63, `workers.dev Worker name is too long: ${config.name.length}`)
})

test('repository configuration excludes the GitHub token', () => {
  const config = JSON.parse(read('infra/cloudflare/production-ingestion-reliability/wrangler.jsonc'))
  const source = read('infra/cloudflare/production-ingestion-reliability/src/index.mjs')
  assert.equal(Object.hasOwn(config.vars, 'GITHUB_ACTIONS_TOKEN'), false)
  assert.match(source, /GITHUB_ACTIONS_TOKEN/)
  assert.doesNotMatch(JSON.stringify(config), /Authorization|Bearer/)
})

test('live deployment evidence preserves the historical reliability closure', () => {
  ...
  assert.match(foundation, /Status: production ingestion reliability closed\./)
  assert.match(runbook, /The production ingestion reliability stage is closed\./)
  assert.match(liveEvidence, /CLOUDFLARE_PRIMARY_REAL_DISPATCH=PASS/)
  assert.match(liveEvidence, /ACTIVE_RUN_DEDUPLICATION=PASS/)
  assert.match(liveEvidence, /GITHUB_SCHEDULED_FALLBACK=PASS/)
  assert.match(liveEvidence, /LIVE_PRODUCTION_RUNTIME_VALIDATION=PASS/)
  assert.match(liveEvidence, /PRODUCTION_INGESTION_RELIABILITY=PASS/)
  assert.doesNotMatch(liveEvidence, /PRODUCTION_INGESTION_RELIABILITY=PENDING/)
})
...
test('closure evidence identities remain consistent across documents', () => {
  ...
  for (const document of documents) {
    for (const evidence of expectedEvidence) assert.match(document, new RegExp(evidence))
    for (const pendingEvidence of forbiddenPendingEvidence) assert.doesNotMatch(document, new RegExp(pendingEvidence))
    assert.match(document, /Evidence ownership and verification boundary/)
  }
  assert.match(readme, /repository-recorded closure evidence/)
  assert.match(readme, /owner-local, non-secret supporting\s+evidence/)
  assert.doesNotMatch(readme, /Diagnosis, recovery commands, architecture boundaries, and immutable run/)
})
