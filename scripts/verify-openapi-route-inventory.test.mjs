...
test('repository route inventory passes', () => {
  ...
  assert.match(output, /SOURCE_PUBLIC_OPERATIONS=40/)
  assert.match(output, /OPENAPI_DOCUMENTED_OPERATIONS=40/)
  assert.match(output, /OPENAPI_MISSING_OPERATIONS=0/)
  assert.match(output, /OPENAPI_ROUTE_INVENTORY=PASS/)
})

test('source inventory exposes exactly 39 GET operations and one POST operation', () => {
  const { publicOperations } = discoverSourceOperations(process.cwd())
  assert.equal(publicOperations.length, 40)
  assert.equal(publicOperations.filter(([method]) => method === 'GET').length, 39)
...
test('complete OpenAPI documents all 40 source operations', () => {
  const { publicOperations } = discoverSourceOperations(process.cwd())
  ...
  assert.equal(openAPIOperations.length, 40)
