...
export const requiredOperations = new Map([
  ...
  ['/api/v1/airports/{icao}/intelligence/congestion', 'getAirportCongestionIntelligence'],
  ...
  ['/api/v1/trajectories/{id}/eta-reliability', 'getETAReliabilityByTrajectoryID'],
...
export function validateOpenAPIContract(spec) {
  ...
  if (spec.openapi !== '3.1.0') errors.push('openapi version must be 3.1.0')
  if (spec.jsonSchemaDialect !== 'https://json-schema.org/draft/2020-12/schema') errors.push('JSON Schema dialect must be draft 2020-12')
  if (spec.info?.title !== 'Global Flight Analytics Public API') errors.push('info.title is not the canonical public API title')
  ...
  for (const server of spec.servers ?? []) {
    if (/^[a-z][a-z0-9+.-]*:\/\//i.test(server.url ?? '')) errors.push(`absolute server URL is forbidden: ${server.url}`)
  }

  const paths = spec.paths ?? {}
  const actualPaths = Object.keys(paths).sort()
  const expectedPaths = [...requiredOperations.keys(), ...requiredMutationOperations.keys()].sort()
  if (JSON.stringify(actualPaths) !== JSON.stringify(expectedPaths)) errors.push(`path surface mismatch: expected ${expectedPaths.join(', ')}`)
  ...
  for (const [route, expectedOperationID] of requiredOperations) {
    ...
    for (const key of Object.keys(pathItem)) {
      if (forbiddenReadMethods.has(key)) errors.push(`public read route ${route} must remain read-only; found ${key}`)
      else if (!allowedReadPathItemKeys.has(key)) errors.push(`unsupported path item key ${key} on ${route}`)
    }
    ...
    if (operation.operationId !== expectedOperationID) errors.push(`operationId mismatch for ${route}`)
    if (operationIds.has(operation.operationId)) errors.push(`duplicate operationId ${operation.operationId}`)
    operationIds.add(operation.operationId)
    if (!operation.responses?.['200']) errors.push(`200 response is missing for ${route}`)
    const params = parameterMap(operation)
    for (const match of route.matchAll(/\{([^}]+)\}/g)) {
      const name = match[1]
      const parameter = params.get(`path:${name}`)
      if (!parameter?.required) errors.push(`required path parameter ${name} is missing for ${route}`)
    }
  }
  ...
  for (const [route, expectedOperationID] of requiredMutationOperations) {
    ...
    if (!pathItem) continue
    for (const key of Object.keys(pathItem)) {
      if (!allowedMutationPathItemKeys.has(key)) errors.push(`unsupported mutation path item key ${key} on ${route}`)
    }
    ...
    operationIds.add(operation.operationId)
    if (!operation.responses?.['200']) errors.push(`200 response is missing for ${route}`)
    if (!operation.responses?.['401']) errors.push(`401 response is missing for protected mutation ${route}`)
    if (!operation.responses?.['503']) errors.push(`503 response is missing for protected mutation ${route}`)
    if (JSON.stringify(operation.security) !== JSON.stringify([{ InternalAPIKey: [] }])) errors.push(`InternalAPIKey security requirement is missing for ${route}`)
    if (operation.requestBody !== undefined) errors.push('Route Intelligence mutation must not publish a request body')
    const params = parameterMap(operation)
    for (const match of route.matchAll(/\{([^}]+)\}/g)) {
      const name = match[1]
      const parameter = params.get(`path:${name}`)
  ...
  if (internalAPIKey?.type !== 'apiKey' || internalAPIKey?.in !== 'header' || internalAPIKey?.name !== 'X-Internal-API-Key') {
  ...
  for (const ref of collectReferences(spec)) {
    ...
    if (pointerResolve(spec, ref) === undefined) errors.push(`unresolved reference: ${ref}`)
  }

  const requiredSchemas = [
    'ErrorResponse','HealthResponse','ReadinessResponse','VersionResponse','RegionItem','AirportListItem','AirportProfile','CurrentTrafficItem','ActiveAircraftMetric','AircraftListItem','AircraftProfile','FlightListItem','FlightProfile','FlightStateItem','Trajectory','AircraftRouteContext','ActiveAircraftMetricResponse','AircraftListResponse','AircraftResponse','FlightListResponse','FlightResponse','FlightStateListResponse','FlightStateResponse','TrajectoryResponse','AircraftRouteContextResponse','TransponderEvidence','CurrentWeather','AnalyticalMetric','AirportIntelligenceRanking','AirportIntelligenceOverview','AirportIntelligenceHistory','AirportIntelligenceTrends','AirportCongestionIntelligence','HistoricalAggregateRecord','HistoricalAggregateHistory','ProjectionIntelligence','StabilityIntelligence','WeatherContext','AirspaceRegionAnalytics','OpenObject','TransponderEvidenceResponse','CurrentWeatherResponse','AnalyticalMetricResponse','AirportIntelligenceRankingResponse','AirportIntelligenceOverviewResponse','AirportIntelligenceHistoryResponse','AirportIntelligenceTrendsResponse','AirportCongestionIntelligenceResponse','HistoricalAggregateRecordResponse','HistoricalAggregateHistoryResponse','ProjectionIntelligenceResponse','StabilityIntelligenceResponse','WeatherContextResponse','AirspaceRegionAnalyticsResponse','ETAReliabilityRoute','ETAReliabilityMetrics','ETAReliabilityNotice','ETAReliability','ETAReliabilityResponse','RouteIntelligenceEvidenceAttribute','RouteIntelligenceEvidence','RouteIntelligenceConfidenceReason','RouteIntelligenceConfidence','RouteIntelligenceLimitation','RouteIntelligenceAirport','RouteIntelligenceEndpoint','RouteIntelligenceWindow','RouteIntelligenceSummary','RouteIntelligenceProvenance','RouteIntelligenceResult','RouteIntelligenceRecord','RouteIntelligenceHistory','RouteIntelligenceRecordResponse','RouteIntelligenceHistoryResponse',
  ]
  for (const schema of requiredSchemas) {
    if (!spec.components?.schemas?.[schema]) errors.push(`required schema is missing: ${schema}`)
  }

  if (spec.components?.schemas?.ErrorResponse?.properties?.success?.const !== false) errors.push('ErrorResponse.success must be const false')

  for (const responseName of [
    'HealthResponse','ReadinessResponse','VersionResponse','RegionResponse','RegionsResponse','AirportResponse','AirportsResponse','CurrentTrafficResponse','ActiveAircraftMetricResponse','AircraftListResponse','AircraftResponse','FlightListResponse','FlightResponse','FlightStateListResponse','FlightStateResponse','TrajectoryResponse','AircraftRouteContextResponse','TransponderEvidenceResponse','CurrentWeatherResponse','AnalyticalMetricResponse','AirportIntelligenceRankingResponse','AirportIntelligenceOverviewResponse','AirportIntelligenceHistoryResponse','AirportIntelligenceTrendsResponse','AirportCongestionIntelligenceResponse','HistoricalAggregateRecordResponse','HistoricalAggregateHistoryResponse','ProjectionIntelligenceResponse','StabilityIntelligenceResponse','WeatherContextResponse','AirspaceRegionAnalyticsResponse','ETAReliabilityResponse','RouteIntelligenceRecordResponse','RouteIntelligenceHistoryResponse',
  ]) {
    const schema = spec.components?.schemas?.[responseName]
    if (schema?.properties?.success?.const !== true || !schema?.properties?.data) errors.push(`${responseName} must use the typed success envelope`)
  }

  const eta = paths['/api/v1/trajectories/{id}/eta-reliability']?.get
  if (eta) {
    if (!eta.responses?.['422']) errors.push('ETA Reliability must document the insufficient-evidence 422 response')
    if (eta.security !== undefined) errors.push('ETA Reliability read must remain unauthenticated')
    const names = new Map((eta.parameters ?? []).map((parameter) => [parameter.name, parameter]))
    if (names.get('as_of_time')?.required !== true) errors.push('ETA Reliability as_of_time must remain required')
    if (names.get('duration_seconds')?.required === true) errors.push('ETA Reliability duration_seconds must remain optional')
  }
...
function requireIncludes(errors, content, literals, label) {
  for (const literal of literals) {
    if (!content.includes(literal)) errors.push(`${label} is missing ${literal}`)
  }
}
...
export function validateRepository(root) {
  ...
  requireIncludes(errors, coreRoutes, ['"/regions"','"/regions/:code"','"/metrics/active-aircraft"','"/traffic/current"','"/aircraft/:icao24/trajectory"','"/aircraft/:icao24/route-context"','"/trajectories/:id"','"/flights/:flightID/states"','"/aircraft/:icao24/latest-state"','"/flights"','"/flights/:id"','"/aircraft"','"/aircraft/:icao24"','"/airports"','"/airports/:icao"'], 'core database routes')

  const response = read(root, 'apps/api/internal/http/response/response.go')
  requireIncludes(errors, response, ['json:"success"', 'json:"data"', 'json:"error"', 'json:"code"', 'json:"message"'], 'response envelope')
  ...
  requireIncludes(errors, regionDTO, ['json:"code"','json:"name"','json:"description"','json:"bounds"','json:"min_latitude"','json:"max_latitude"','json:"min_longitude"','json:"max_longitude"'], 'region DTO')
  ...
  requireIncludes(errors, trafficDTO, ['json:"icao24"','json:"altitude_m"','json:"altitude_status"','json:"altitude_source"','json:"observed_at"','json:"position_observed_at"','json:"message_observed_at"'], 'traffic DTO')
  ...
  const etaReliabilityRoutes = read(root, 'apps/api/internal/server/eta_reliability_routes.go')
  requireIncludes(errors, etaReliabilityRoutes, ['ETAReliabilityPath = "/trajectories/:id/eta-reliability"','v1.Get(ETAReliabilityPath, handler.GetByTrajectoryID)'], 'ETA Reliability routes')
  const etaReliabilityDTO = read(root, 'apps/api/internal/http/dto/eta_reliability.go')
  requireIncludes(errors, etaReliabilityDTO, ['json:"eligible_sample_count"','json:"metrics,omitempty"','json:"evidence_class"','json:"input_fingerprint"'], 'ETA Reliability DTO')
  const etaReliabilityHandler = read(root, 'apps/api/internal/http/handlers/eta_reliability.go')
  requireIncludes(errors, etaReliabilityHandler, ['"ETA_RELIABILITY_UNAVAILABLE"','fiber.StatusUnprocessableEntity','projectionIntelligenceAsOfTimeQuery','projectionIntelligenceDurationSecondsQuery'], 'ETA Reliability handler')
  ...
  for (const [name, command] of Object.entries(scripts)) {
    if (packageJSON.scripts?.[name] !== command) errors.push(`package script is missing or incorrect: ${name}`)
  }

  const release = read(root, 'scripts/verify-release.sh')
  requireIncludes(errors, release, ['pnpm run test:openapi-contract','pnpm run verify:openapi-contract','RELEASE_OPENAPI_CONTRACT=PASS'], 'release gate')
  const workflow = read(root, '.github/workflows/api-contract.yml')
  requireIncludes(errors, workflow, ['name: OpenAPI Contract','node --test scripts/verify-openapi-contract.test.mjs','node scripts/verify-openapi-contract.mjs','openapi/openapi.json','actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a'], 'OpenAPI workflow')
  const document = read(root, 'docs/175_OPENAPI_CONTRACT_FOUNDATION.md')
  requireIncludes(errors, document, ['Status: IMPLEMENTED','thirty-eight production public operations','InternalAPIKey','OPENAPI_CONTRACT=PASS'], 'Document 175')
  const closureDocument = read(root, 'docs/180_OPENAPI_ROUTE_INTELLIGENCE_CONTRACT_CLOSURE.md')
  requireIncludes(errors, closureDocument, ['Status: IMPLEMENTED','InternalAPIKey','OPENAPI_MISSING_OPERATIONS=0','b67005b7ff6a944cffc2f4d846aec1009ea69e53'], 'Document 180')
  const documentIndex = read(root, 'docs/DOCUMENT_INDEX.md')
  requireIncludes(errors, documentIndex, ['OPENAPI-CONTRACT-FOUNDATION-V1:DOCUMENT-INDEX','175_OPENAPI_CONTRACT_FOUNDATION.md','OPENAPI-ROUTE-INTELLIGENCE-CONTRACT-CLOSURE-V1:DOCUMENT-INDEX','180_OPENAPI_ROUTE_INTELLIGENCE_CONTRACT_CLOSURE.md'], 'documentation index')
...
function main() {
  ...
  if (errors.length > 0) {
    for (const error of errors) console.error(`OPENAPI_CONTRACT=FAIL reason=${error}`)
    process.exit(1)
  }
  console.log('OPENAPI_CONTRACT_PATHS=40')
