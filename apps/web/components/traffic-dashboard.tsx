...
import { buildStabilityAsOfTimes,useStabilityIntelligence } from '@/lib/queries/stability-intelligence'
...
export function TrafficDashboard({regions,selectedRegion,onSelectedRegionCodeChange,initialTraffic,initialError,regionsWarning}:TrafficDashboardProps){
  ...
  const weatherContextQuery=useWeatherContext(projectionTrajectoryID,projectionAsOfTime)
  const stabilityAsOfTimes=buildStabilityAsOfTimes(trajectoryQuery.data?.start_time??null,trajectoryQuery.data?.end_time??null)
  const stabilityQuery=useStabilityIntelligence(projectionTrajectoryID,stabilityAsOfTimes)
  ...
  return <>
    ...
    <section className='mt-8 rounded-xl border border-slate-800 bg-slate-900 p-4 sm:p-6'><h2 className='text-xl font-semibold'>Current Traffic — {selectedRegion.name}</h2><p className='mt-2 text-sm text-slate-400'>Select an aircraft marker to inspect its live state, preliminary route context, validated production Route Intelligence, research Projection Intelligence, Weather Context, Stability and Explainability, registered profile, persisted trajectory and quality limitations.</p><div className='mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_480px]'><div aria-busy={trafficQuery.isFetching}><TrafficMap aircraft={traffic} region={selectedRegion} selectedAircraftICAO24={selectedAircraftICAO24} trajectory={trajectoryQuery.data} projection={projectionQuery.data?.projection} onSelectAircraft={(icao24: string)=>setSelectedAircraftICAO24(normalizeICAO24(icao24))}/></div><div className='space-y-4'><AircraftExplorer aircraft={traffic} selectedAircraftICAO24={selectedAircraftICAO24} onSelectAircraft={(icao24: string)=>setSelectedAircraftICAO24(normalizeICAO24(icao24))} isFetching={trafficQuery.isFetching}/><AircraftDetailPanel selectedICAO24={selectedAircraftICAO24} aircraft={selectedAircraft} routeContext={routeContextQuery.data} routeContextIsPending={routeContextQuery.isPending} routeContextIsFetching={routeContextQuery.isFetching} routeContextError={routeContextQuery.error} onRetryRouteContext={()=>{void routeContextQuery.refetch()}} trajectory={trajectoryQuery.data} trajectoryIsPending={trajectoryQuery.isPending} trajectoryIsFetching={trajectoryQuery.isFetching} trajectoryError={trajectoryQuery.error} onRetryTrajectory={()=>{void trajectoryQuery.refetch()}} onClose={()=>setSelectedAircraftICAO24(null)}/><RouteIntelligencePanel selectedICAO24={selectedAircraftICAO24} trajectoryID={routeIntelligenceTrajectoryID} record={routeIntelligenceQuery.data} isPending={routeIntelligenceQuery.isPending} isFetching={routeIntelligenceQuery.isFetching} error={routeIntelligenceQuery.error} onRetry={()=>{void routeIntelligenceQuery.refetch()}}/><ProjectionIntelligencePanel selectedICAO24={selectedAircraftICAO24} trajectoryID={projectionTrajectoryID} result={projectionQuery.data} isPending={projectionQuery.isPending} isFetching={projectionQuery.isFetching} error={projectionQuery.error} onRetry={()=>{void projectionQuery.refetch()}}/><WeatherContextPanel selectedICAO24={selectedAircraftICAO24} trajectoryID={projectionTrajectoryID} result={weatherContextQuery.data} isPending={weatherContextQuery.isPending} isFetching={weatherContextQuery.isFetching} error={weatherContextQuery.error} onRetry={()=>{void weatherContextQuery.refetch()}}/><StabilityIntelligencePanel selectedICAO24={selectedAircraftICAO24} trajectoryID={projectionTrajectoryID} asOfTimes={stabilityAsOfTimes} result={stabilityQuery.data} isPending={stabilityQuery.isPending} isFetching={stabilityQuery.isFetching} error={stabilityQuery.error} onRetry={()=>{void stabilityQuery.refetch()}}/></div></div>{!trafficQuery.isFetching&&!errorMessage&&traffic.length===0?<p className='mt-4 text-sm text-slate-400'>No aircraft were returned for the selected region.</p>:null}</section>
  </>
}
