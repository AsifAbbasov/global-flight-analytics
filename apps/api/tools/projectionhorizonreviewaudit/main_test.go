...
func TestRepositoryProjectionHorizonRequirementsPass(t *testing.T) {
	...
	if len(failures) != 0 {
		t.Fatalf(
			"projection horizon review requirements failed:\n%s",
			strings.Join(failures, "\n"),
		)
	}
}

func TestInspectRequirementsReportsMissingAndForbiddenFragments(
	t *testing.T,
) {
	root := t.TempDir()
	...
	if !strings.Contains(failures[0]+failures[1], `misses "missing"`) {
		t.Fatalf("missing-fragment failure not reported: %#v", failures)
	}
	if !strings.Contains(
		failures[0]+failures[1],
		`retains forbidden "forbidden"`,
	) {
