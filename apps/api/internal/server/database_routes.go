...
func registerDatabaseRoutes(v1 fiber.Router, dbPool *pgxpool.Pool, openMeteoTimeout time.Duration) error {
	h := buildDatabaseRouteHandlers(dbPool)
	composition, err := routepipeline.NewPostgres(routepipeline.PostgresConfig{Pool: dbPool})
	if err != nil {
		return fmt.Errorf("compose production Route Intelligence pipeline: %w", err)
	}
	h.routeIntelligence = handlers.NewRouteIntelligenceHandler(composition.Pipeline, composition.Store)
	if err := registerWeatherRoute(v1, dbPool, openMeteoTimeout); err != nil {
		return fmt.Errorf("register weather route: %w", err)
	}
	...
	v1.Get("/traffic/current", h.traffic.GetCurrent)
	v1.Get("/aircraft/:icao24/trajectory", h.trajectory.GetLatestByICAO24)
	v1.Get("/aircraft/:icao24/route-context", h.routeContext.GetByICAO24)
	v1.Get("/trajectories/:id", h.trajectory.GetByID)
	v1.Post("/trajectories/:id/route-intelligence", h.routeIntelligence.ProcessByTrajectoryID)
	v1.Get("/trajectories/:id/route-intelligence/latest", h.routeIntelligence.GetLatestByTrajectoryID)
	v1.Get("/trajectories/:id/route-intelligence/history", h.routeIntelligence.ListHistoryByTrajectoryID)
	v1.Get("/flights/:flightID/states", h.flightState.ListByFlightID)
	v1.Get("/aircraft/:icao24/latest-state", h.flightState.GetLatestByICAO24)
	v1.Get("/flights", h.flight.List)
	v1.Get("/flights/:id", h.flight.GetByID)
	v1.Get("/aircraft", h.aircraft.List)
	v1.Get("/aircraft/:icao24", h.aircraft.GetByICAO24)
	v1.Get("/airports", h.airport.List)
	v1.Get("/airports/:icao", h.airport.GetByICAO)
	return nil
}

func buildDatabaseRouteHandlers(dbPool *pgxpool.Pool) databaseRouteHandlers {
	...
	stateRepository := postgres.NewFlightStateRepository(dbPool)
	stateService := flightstate.NewService(stateRepository)
	stateHandler := handlers.NewFlightStateHandler(stateService)
	...
	metricsService := metrics.NewService(metricsRepository, regionService)
	...
	trajectoryService := trafficquery.New(trafficquery.Config{TrajectoryRepository: trajectoryRepository})
	trajectoryHandler := handlers.NewTrajectoryHandler(trajectoryService)
	routeContextService := trafficroutecontext.New(trafficroutecontext.Config{TrajectoryReader: trajectoryService, AirportLister: airportService})
	routeContextHandler := handlers.NewRouteContextHandler(routeContextService)
	return databaseRouteHandlers{airport: airportHandler, aircraft: aircraftHandler, flight: flightHandler, flightState: stateHandler, metrics: metricsHandler, region: regionHandler, routeContext: routeContextHandler, traffic: trafficHandler, trajectory: trajectoryHandler}
}
