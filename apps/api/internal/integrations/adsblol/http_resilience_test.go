...
type failingADSBLOLResponseObserver struct {
	err error
}

func (observer failingADSBLOLResponseObserver) ObserveProviderResponse(
	string,
	int,
	http.Header,
	time.Duration,
...
func TestServerFailurePreservesProviderClassificationWhenObserverFails(
	...
	client, err := NewClientWithResponseObserver(
		...
		failingADSBLOLResponseObserver{err: observerErr},
	)
	if err != nil {
		t.Fatalf("create adsb.lol client: %v", err)
	}
