...
PROJECTION_ROUTE_FREQUENCY_PERMANENT_AUDIT_GITHUB_ACTIONS_RUN=PENDING
PROJECTION_ROUTE_FREQUENCY_REVIEW_STATUS=OPEN
```

The review remains open because exact Continuous Integration for the policy commit,
the permanent-audit commit, and the final closure commit must still be recorded.
