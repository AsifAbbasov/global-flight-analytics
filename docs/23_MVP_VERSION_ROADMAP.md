...
It is not a commercial flight tracker clone. It is not operational aviation software. It is not an air traffic control decision-support tool.
...
These tables are enough to support the first reliable vertical slice.

---

## 8. MVP Backend Packages

The MVP backend should be organized around these package groups:
...
No operational aviation claims anywhere in the product.
```
...
Trajectory construction works
Data quality scoring works
Basic route inference works
Aircraft detail panel explains data limitations
```
...
```text
Feature Store works
Historical pattern library works
Replay evaluation works
Route confidence is explainable
Short projection is conservative and confidence-scored
```

### Version 2 success

```text
Advanced similarity works
Local traffic scene works
Airspace complexity works
Forecast stability works
Region and airport intelligence are visible in dashboards
```
...
The project must grow in this order:
...
Research Backlog = heavy models, satellite fusion, climate models, operational systems
```
