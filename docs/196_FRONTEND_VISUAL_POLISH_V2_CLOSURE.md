...
## Historical stale preview deployment notification

Earlier on 2026-08-23, Vercel sent an automated failed-preview notification for the stale
feature branch:
...
The supplied notification/log excerpt ends after the build entered TypeScript validation,
so no specific compiler diagnostic is asserted here.

This notification was generated by Vercel; it was **not** an outbound support email from
the project. It is historical evidence for an earlier feature-branch preview only. It must
not be interpreted as current production state or as evidence against the later exact-head
frontend closure.
...
Later frontend-governance merges were also recorded as Vercel `success`; therefore the
stale `a2c941a...` preview failure is superseded for current frontend deployment truth.
